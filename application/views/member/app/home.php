<div class="alert alert-info alert-dismissible" role="alert" style="text-align: justify;">
	<i class="fa fa-warning"></i> <strong>Selamat Datang</strong> di Aplikasi Timses ID</a>
</div>
<div class="page-content-inner">
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="dashboard-stat2 ">
				<div class="display">
					<div class="number">
						<h3 class="font-green-sharp">
							<span data-counter="counterup" data-value="<?= count($data_relawan); ?>"><?= number_format(count($data_relawan),0); ?>/ <?= number_format($get_info->target_relawan,0); ?></span>
							<small class="font-green-sharp">Orang</small>
						</h3>
						<small>Relawan</small>
					</div>
					<div class="icon">
						<i class="icon-users"></i>
					</div>
				</div>
				<?php
				$hitung_persentase_relawan = count($data_relawan) / $get_info->target_relawan * 100;
				?>
				<div class="progress-info">
					<div class="progress">
						<span style="width: <?= number_format($hitung_persentase_relawan,2); ?>%;" class="progress-bar progress-bar-success green-sharp">
							<span class="sr-only"><?= number_format($hitung_persentase_relawan,2); ?>% Progress</span>
						</span>
					</div>
					<div class="status">
						<div class="status-title"> Progress </div>
						<div class="status-number"> <?= number_format($hitung_persentase_relawan,2); ?>% </div>
					</div>
				</div>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="dashboard-stat2 ">
				<div class="display">
					<div class="number">
						<h3 class="font-red-haze">
							<span data-counter="counterup" data-value="<?= count($data_rekrutmen); ?>"><?= count($data_rekrutmen); ?>/ <?= number_format($get_info->target_rekrutmen,0); ?></span>
							<small class="font-red-haze">Pemilih</small>
						</h3>
						<small>REKRUTMEN</small>
					</div>
					<div class="icon">
						<i class="icon-user-following"></i>
					</div>
				</div>
				<?php
				$hitung_persentase_rekrutmen = count($data_rekrutmen) / $get_info->target_rekrutmen * 100;
				?>
				<div class="progress-info">
					<div class="progress">
						<span style="width: <?= number_format($hitung_persentase_rekrutmen,2); ?>%;" class="progress-bar progress-bar-success red-haze">
							<span class="sr-only"><?= number_format($hitung_persentase_rekrutmen,2); ?>% Progress</span>
						</span>
					</div>
					<div class="status">
						<div class="status-title"> Progress </div>
						<div class="status-number"> <?= number_format($hitung_persentase_rekrutmen,2); ?>% </div>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-12 col-sm-12">
			<div class="portlet light tasks-widget">
				<div class="portlet-title">
					<div class="caption caption-md">
						<i class="icon-bar-chart font-dark hide"></i>
						<span class="caption-subject font-dark bold ">Jumlah Relawan</span>
						<span class="caption-helper"><?= number_format(count($data_relawan),0); ?> Orang</span>
					</div>
				</div>
				<div class="portlet-body">
					<div class="task-content">
						<div class="table-scrollable table-scrollable-borderless">
							<table class="table table-hover table-light">
								<thead>
									<!-- <tr class="">
										<th colspan="2"> Nama Relawan </th>
										<th> NIK </th>
										<th> No. HP </th>
										<th> Pekerjaan </th>
										<th> Persentase Realisasi </th>
									</tr> -->
									<tr class="">
										<th colspan="2"> Nama Relawan </th>
										<th> NIK </th>
										<th> No. HP </th>
										<th> Pekerjaan </th>
										<th> Jumlah Rekrutmen </th>
									</tr>
								</thead>
								<?php
								$no = 1;
								foreach ($data_relawan as $key => $value) {
									if($no>5){
										echo'';
									}else{
										$url2 = 'http://pradi.is-very-good.org:7733/api/relawanprofiles/getProfile/'.$value['idRelawan'];
										$data_r = $this->Main_model->getAPI($url2);
										$url3 = 'http://pradi.is-very-good.org:7733/api/rekrutmen/relawan/'.$value['idRelawan'];
										$data_re = $this->Main_model->getAPI($url3);
										echo'
										<tr>
											<td class="fit">
												<img class="user-pic rounded" src="data:image/png;base64,"> </td>
											<td>
												<a href="javascript:;" class="primary-link">'.$data_r['namaRelawan'].'</a>
											</td>
											<td> '.$data_r['NIK'].'</td>
											<td> '.$value['telepon'].' </td>
											<td> '.$data_r['pekerjaan'].' </td>
											<td> '.number_format(count($data_re),0).' Orang </td>
										</tr>
										';
									}
									$no++;
								}
								?>
								<!-- <tr>
									<td class="fit">
										<img class="user-pic rounded" src="data:image/png;base64,"> </td>
									<td>
										<a href="javascript:;" class="primary-link">Mukhammad Fakhir Rizal</a>
									</td>
									<td> 14 Tugas </td>
									<td> 4 Tugas </td>
									<td> 10 Tugas </td>
									<td>
										<span class="bold theme-font">80%</span>
									</td>
								</tr>
								<tr>
									<td class="fit">
										<img class="user-pic rounded" src="data:image/png;base64,"> </td>
									<td>
										<a href="javascript:;" class="primary-link">Elad Oktarizo</a>
									</td>
									<td> 15 Tugas </td>
									<td> 12 Tugas </td>
									<td> 3 Tugas </td>
									<td>
										<span class="bold theme-font">67%</span>
									</td>
								</tr>
								<tr>
									<td class="fit">
										<img class="user-pic rounded" src="data:image/png;base64,"> </td>
									<td>
										<a href="javascript:;" class="primary-link">Aris Riyanto</a>
									</td>
									<td> 13 Tugas </td>
									<td> 4 Tugas </td>
									<td> 9 Tugas </td>
									<td>
										<span class="bold theme-font">98%</span>
									</td>
								</tr>
								<tr>
									<td class="fit">
										<img class="user-pic rounded" src="data:image/png;base64,"> </td>
									<td>
										<a href="javascript:;" class="primary-link">Estio Nurcahyanto</a>
									</td>
									<td> 6 Tugas </td>
									<td> 5 Tugas </td>
									<td> 1 Tugas </td>
									<td>
										<span class="bold theme-font">58%</span>
									</td>
								</tr> -->
							</table>
						</div>
					</div>
					<?php
					if(count($data_relawan)>5){
						echo'
						<div class="task-footer">
							<div class="btn-arrow-link pull-right">
								<a href="'.site_url('member_side/relawan').'">Lihat Semua</a>
								<i class="icon-arrow-right"></i>
							</div>
						</div>
						';
					}else{
						echo'';
					}
					?>
				</div>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12 col-sm-12">
			<div class="portlet light tasks-widget">
				<div class="portlet-title">
					<div class="caption caption-md">
						<i class="icon-bar-chart font-dark hide"></i>
						<span class="caption-subject font-dark bold">Calon Pemiih/ Rekrutmen</span>
						<span class="caption-helper"><?= number_format(count($data_rekrutmen),0); ?> Orang</span>
					</div>
				</div>
				<div class="portlet-body">
					<div class="task-content">
						<div class="scroller" style="height: 338px;" data-always-visible="1" data-rail-visible1="0" data-handle-color="#D7DCE2">
							<div class="general-item-list">
								<?php
								foreach ($data_rekrutmen as $key => $value) {
								?>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSlVMT-lQ0Aoynjhf-DmiWZffgym7Xozqpg-gH1qhAwtrrBCaW4dA">
											<a href="" class="item-name primary-link"><?= $value['namaRekrutmen']; ?></a>
										</div>
										<span class="item-status">
											<?php
											if($value['isVerified']==true){
												echo'<span class="badge badge-empty badge-success"></span> Terverifikasi';
											}else{
												echo'<span class="badge badge-empty badge-warning"></span> Belum Terverifikasi';
											}
											?>
											
										</span>
									</div>
									<div class="item-body">
										<b>NIK</b> : <?= $value['NIK']; ?><br>
										<b>No. HP</b> : <?= $value['telepon']; ?>
									</div>
								</div>
								<?php
								}
								?>
								<!-- <div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSlVMT-lQ0Aoynjhf-DmiWZffgym7Xozqpg-gH1qhAwtrrBCaW4dA">
											<a href="" class="item-name primary-link">Ruud van Nistelrooy</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-danger"></span> Pemilih Loyal</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://ondemand.construx.com/wp-content/uploads/2017/04/individual-ondemand-geek2.png">
											<a href="" class="item-name primary-link">Robert Lewandowski</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-warning"></span> Swing Voter</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://cdn3.iconfinder.com/data/icons/business-round-flat-vol-1-1/36/user_account_profile_avatar_person_male-512.png">
											<a href="" class="item-name primary-link">Nick Larson</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-success"></span> Pemilih Loyal</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://cdn3.iconfinder.com/data/icons/business-round-flat-vol-1-1/36/user_account_profile_avatar_person_businessman_male-512.png">
											<a href="" class="item-name primary-link">Roberto Firmino</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-success"></span> Pemilih Loyal</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://cdn.iconscout.com/icon/premium/png-512-thumb/businessman-9-132135.png">
											<a href="" class="item-name primary-link">Richard Stone</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-success"></span> Pemilih Loyal</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://cdn2.iconfinder.com/data/icons/office-and-business-special-set-1/260/23-512.png">
											<a href="" class="item-name primary-link">David Beckham</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-success"></span> Pemilih Loyal</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div>
								<div class="item">
									<div class="item-head">
										<div class="item-details">
											<img class="item-pic rounded" src="https://cdn4.iconfinder.com/data/icons/avatars-21/512/avatar-circle-human-male-2-512.png">
											<a href="" class="item-name primary-link">Zlatan Ibrahimović</a>
										</div>
										<span class="item-status">
											<span class="badge badge-empty badge-success"></span> Pemilih Loyal</span>
									</div>
									<div class="item-body"> <b>Alamat</b> : Jln. dr. Cipto 61, Proyonanggan Tengah, Batang 51211<br><b>No. HP</b> : +62 856 9630 3627 </div>
								</div> -->
							</div>
						</div>
					</div>
					<!-- <div class="task-footer">
						<div class="btn-arrow-link pull-right">
							<a href="#">Lihat Semua</a>
							<i class="icon-arrow-right"></i>
						</div>
					</div> -->
				</div>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12 col-sm-12">
			<!-- <div id="containerrr" style="min-width: 310px; height: 400px; margin: 0 auto"></div> -->
			<script type="text/javascript" src="http://code.jquery.com/jquery-1.11.3.min.js"></script>
			<script src="https://code.highcharts.com/highcharts.js"></script>
			<script src="https://code.highcharts.com/modules/data.js"></script>
			<script src="https://code.highcharts.com/modules/drilldown.js"></script>
			<script src="https://code.highcharts.com/modules/exporting.js"></script>
			<script src="https://code.highcharts.com/modules/export-data.js"></script>
			<script src="https://code.highcharts.com/modules/accessibility.js"></script>
			<!-- <script src="<?=base_url('assets/js/dark.js');?>"></script> -->
			<script>
				Highcharts.chart('containerrr', {
					chart: {
						type: 'column'
					},
					credits: {
					enabled: false
					}, 
					title: {
						text: 'Grafik Persebaran Relawan'
					},
					subtitle: {
						text: 'Kabupaten Batang'
					},
					accessibility: {
						announceNewData: {
							enabled: true
						}
					},
					xAxis: {
						type: 'category'
					},
					yAxis: {
						title: {
							text: 'Jumlah Relawan'
						}

					},
					legend: {
						enabled: false
					},
					plotOptions: {
						series: {
							borderWidth: 0,
							dataLabels: {
								enabled: true,
								format: '{point.y}'
							}
						}
					},

					tooltip: {
						headerFormat: '<span style="font-size:11px">{series.name}</span><br>',
						pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y}</b> Relawan<br/>'
					},

					series: [
						{
							name: "Relawan",
							colorByPoint: true,
							data: [
								{
									name: "Kec. Batang",
									y: 12,
									drilldown: null
								},
								{
									name: "Kec. Tulis",
									y: 10,
									drilldown: null
								},
								{
									name: "Kec. Bandar",
									y: 7,
									drilldown: null
								},
								{
									name: "Kec. Blado",
									y: 5,
									drilldown: null
								},
								{
									name: "Kec. Subah",
									y: 4,
									drilldown: null
								},
								{
									name: "Kec. Gringsing",
									y: 11,
									drilldown: null
								},
								{
									name: "Kec. Pecalungan",
									y: 7,
									drilldown: null
								}
							]
						}
					],
				});
			</script>
		</div>
	</div>
	<hr>
	<div class="row">
		<div class="col-md-12 col-sm-12">
			<!-- <div id="containerrrr" style="min-width: 310px; height: 400px; margin: 0 auto"></div> -->
			<script>
				Highcharts.chart('containerrrr', {
					chart: {
						type: 'column'
					},
					credits: {
					enabled: false
					}, 
					title: {
						text: 'Grafik Persebaran Rekrutmen'
					},
					subtitle: {
						text: 'Kabupaten Batang'
					},
					accessibility: {
						announceNewData: {
							enabled: true
						}
					},
					xAxis: {
						type: 'category'
					},
					yAxis: {
						title: {
							text: 'Jumlah Rekrutmen'
						}

					},
					legend: {
						enabled: false
					},
					plotOptions: {
						series: {
							borderWidth: 0,
							dataLabels: {
								enabled: true,
								format: '{point.y}'
							}
						}
					},

					tooltip: {
						headerFormat: '<span style="font-size:11px">{series.name}</span><br>',
						pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y}</b> Calon Pemilih<br/>'
					},

					series: [
						{
							name: "Rekrutmen",
							colorByPoint: true,
							data: [
								{
									name: "Kec. Batang",
									y: 62,
									drilldown: null
								},
								{
									name: "Kec. Tulis",
									y: 30,
									drilldown: null
								},
								{
									name: "Kec. Bandar",
									y: 27,
									drilldown: null
								},
								{
									name: "Kec. Blado",
									y: 25,
									drilldown: null
								},
								{
									name: "Kec. Subah",
									y: 24,
									drilldown: null
								},
								{
									name: "Kec. Gringsing",
									y: 40,
									drilldown: null
								},
								{
									name: "Kec. Pecalungan",
									y: 27,
									drilldown: null
								}
							]
						}
					],
				});
			</script>
		</div>
	</div>
</div>