...
class Master extends CI_Controller {
	...
	public function target_suara_kecamatan(){
		$data['parent'] = 'master';
		$data['child'] = 'target_suara';
		$data['grand_child'] = '';
		// $data['data_tabel'] = $this->Main_model->getSelectedData('kube a', 'a.*', array('a.deleted'=>'0'), "a.fullname ASC")->result();
		$this->load->view('member/template/header',$data);
		$this->load->view('member/master/target_suara_kecamatan',$data);
		$this->load->view('member/template/footer');
	}
	...
	public function update_target_suara(){
		...
		// );
		// // print_r($data1);
		// $this->Main_model->insertData('target_suara',$data1);

		$this->Main_model->log_activity($this->session->userdata('id'),'Updating data',"Update target suara (".$this->input->post('target_suara').")",$this->session->userdata('location'));
		...
		if($hasil_insert>0){
			...
			echo "<script>window.location='".base_url()."member_side/ubah_target_suara'</script>";
		}
		else{
	...
	public function saksi_data()
	{
		...
		// $data['data_tabel'] = $this->Main_model->getSelectedData('kube a', 'a.*', array('a.deleted'=>'0'), "a.fullname ASC")->result();
		$this->load->view('member/template/header',$data);
		$this->load->view('member/master/saksi_data',$data);
		$this->load->view('member/template/footer');
	}
	public function json_saksi_data(){
		$get_info = $this->Main_model->getSelectedData('user_to_role a', 'a.*', array('a.user_id'=>$this->session->userdata('id')))->row();
		$url1 = 'http://kertasfolio.id:99/api/userdatas/event/'.$get_info->id_event;
		$data = $this->Main_model->getAPI($url1);
		$data_tampil = array();
		...
		foreach ($data as $key => $value) {
			...
			$isi['nama'] = $value['namaUser'];
			$role = '';
			if($value['roleUser']=='54940bb1-6bdf-42e5-af13-3a305fd53cae'){
				$role = 'SUPERADMIN';
			}elseif($value['roleUser']=='ef60b7ba-e255-4747-aad7-48909d08c0e0'){
				$role = 'ADMIN KECAMATAN';
			}elseif($value['roleUser']=='8d5d0ee6-d80c-44d3-a611-5c806f6980a3'){
				$role = 'ADMIN PROVINSI';
			}elseif($value['roleUser']=='ea945254-7880-4453-b02d-97df2999cba9'){
				$role = 'ADMIN PILKADA';
			}elseif($value['roleUser']=='e1481905-ba62-4f14-a078-982d3d59b680'){
				$role = 'ADMIN KABUPATEN';
			}else{echo'';}
			$isi['keterangan'] = $role;
			$wilayah = '';
	...
	public function add_saksi_data()
	{
		$data['parent'] = 'master';
		$data['child'] = 'saksi';
		$data['grand_child'] = '';
		$data['get_info'] = $this->Main_model->getSelectedData('user_to_role a', 'a.*', array('a.user_id'=>$this->session->userdata('id')))->row();
		$this->load->view('member/template/header',$data);
		$this->load->view('member/master/add_saksi_data',$data);
		$this->load->view('member/template/footer');
	}
	public function save_saksi_data(){
		$r_u = '';
		$ket = '';
		$role_id = '';
		if($this->input->post('role_name')=='1a' OR $this->input->post('role_name')=='1b'){
			...
			$r_u = 'e1481905-ba62-4f14-a078-982d3d59b680';
			$ket = 'ADMIN KABUPATEN';
			$role_id = '3';
		}elseif($this->input->post('role_name')=='3a' OR $this->input->post('role_name')=='3b' OR $this->input->post('role_name')=='3c'){
			$r_u = 'ef60b7ba-e255-4747-aad7-48909d08c0e0';
			$ket = 'ADMIN KECAMATAN';
			$role_id = '4';
		}else{echo'';}
		$url_insert = 'http://kertasfolio.id:99/api/userdatas/register';
	...
	public function edit_saksi_data()
	{
		$data['parent'] = 'master';
		$data['child'] = 'saksi';
		$data['grand_child'] = '';
		...
		$this->load->view('member/template/header',$data);
		$this->load->view('member/master/edit_saksi_data',$data);
		$this->load->view('member/template/footer');
	}
	/* Task */
	/* Other Function */
